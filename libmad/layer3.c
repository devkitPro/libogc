 ...
 *
 * $Id: layer3.c,v 1.1.1.1 2004-08-03 13:56:55 wntrmute Exp $
 */
...
struct sideinfo {
  ...
  struct granule {
    struct channel {
      ...
      /* from main_data */
      u8 scalefac[39];	/* scalefac_l and/or scalefac_s */
    } ch[2];
  } gr[2];
};
 ...
 */
static
u8 const sfb_48000_long[] = {
   4,  4,  4,  4,  4,  4,  6,  6,  6,   8,  10,
  12, 16, 18, 22, 28, 34, 40, 46, 54,  54, 192
};

static
u8 const sfb_44100_long[] = {
   4,  4,  4,  4,  4,  4,  6,  6,  8,   8,  10,
  12, 16, 20, 24, 28, 34, 42, 50, 54,  76, 158
};

static
u8 const sfb_32000_long[] = {
   4,  4,  4,  4,  4,  4,  6,  6,  8,  10,  12,
  16, 20, 24, 30, 38, 46, 56, 68, 84, 102,  26
};

static
u8 const sfb_48000_short[] = {
 ...
 */
static
u8 const sfb_24000_long[] = {
   6,  6,  6,  6,  6,  6,  8, 10, 12,  14,  16,
  18, 22, 26, 32, 38, 46, 54, 62, 70,  76,  36
};

static
u8 const sfb_22050_long[] = {
   6,  6,  6,  6,  6,  6,  8, 10, 12,  14,  16,
 ...
 */
static
enum mad_error III_sideinfo(struct mad_bitptr *ptr, u32 nch,
			    s32 lsf, struct sideinfo *si,
			    u32 *data_bitlen,
			    u32 *priv_bitlen)
{
  u32 ngr, gr, ch, i;
 ...
 */
static
u32 III_scalefactors_lsf(struct mad_bitptr *ptr,
				  struct channel *channel,
				  struct channel *gr1ch, s32 mode_extension)
{
  struct mad_bitptr start;
  u32 scalefac_compress, index, slen[4], part, n, i;
  u8 const *nsfb;
 ...
 */
static
u32 III_scalefactors(struct mad_bitptr *ptr, struct channel *channel,
			      struct channel const *gr0ch, u32 scfsi)
{
  struct mad_bitptr start;
  u32 slen1, slen2, sfbi;
...
void III_exponents(struct channel const *channel,
		   u8 const *sfbwidth, s32 exponents[39])
{
  s32 gain;
  u32 scalefac_multiplier, sfbi;

  gain = (s32) channel->global_gain - 210;
  scalefac_multiplier = (channel->flags & scalefac_scale) ? 2 : 1;

  if (channel->block_type == 2) {
    u32 l;
    s32 gain0, gain1, gain2;
    ...
    if (channel->flags & mixed_block_flag) {
      ...
      while (l < 36) {
	exponents[sfbi] = gain -
	  (s32) ((channel->scalefac[sfbi] + (pretab[sfbi] & premask)) <<
			scalefac_multiplier);
    ...
    gain0 = gain - 8 * (s32) channel->subblock_gain[0];
    gain1 = gain - 8 * (s32) channel->subblock_gain[1];
    gain2 = gain - 8 * (s32) channel->subblock_gain[2];

    while (l < 576) {
      exponents[sfbi + 0] = gain0 -
	(s32) (channel->scalefac[sfbi + 0] << scalefac_multiplier);
      exponents[sfbi + 1] = gain1 -
	(s32) (channel->scalefac[sfbi + 1] << scalefac_multiplier);
      exponents[sfbi + 2] = gain2 -
	(s32) (channel->scalefac[sfbi + 2] << scalefac_multiplier);
	...
	exponents[sfbi] = gain -
	  (s32) ((channel->scalefac[sfbi] + pretab[sfbi]) <<
			scalefac_multiplier);
      }
    }
    else {
      for (sfbi = 0; sfbi < 22; ++sfbi) {
	exponents[sfbi] = gain -
	  (s32) (channel->scalefac[sfbi] << scalefac_multiplier);
      }
    }
  }
}
 ...
 */
static
mad_fixed_t III_requantize(u32 value, s32 exp)
{
...
enum mad_error III_huffdecode(struct mad_bitptr *ptr, mad_fixed_t xr[576],
			      ...
			      u32 part2_length)
{
  s32 exponents[39], exp;
  s32 const *expptr;
  struct mad_bitptr peek;
  s32 bits_left, cachesz;
  register mad_fixed_t *xrptr;
  mad_fixed_t const *sfbound;
  register u32 bitcache;
  ...
  /* big_values */
  {
    ...
    u32 linbits, startbits, big_values, reqhits;
...
void III_reorder(mad_fixed_t xr[576], struct channel const *channel,
		 u8 const sfbwidth[39])
{
  mad_fixed_t tmp[32][3][6];
  u32 sb, l, f, w, sbw[3], sw[3];
			  ...
			  u8 const *sfbwidth)
{
  ...
  if (header->mode_extension & I_STEREO) {
    ...
    if (header->flags & MAD_FLAG_LSF_EXT) {
      u8 const *illegal_pos = granule[1].ch[1].scalefac;
 ...
 */
static
void III_aliasreduce(mad_fixed_t xr[576], s32 lines)
{
...
# if defined(ASO_IMDCT)
void III_imdct_l(mad_fixed_t const [18], mad_fixed_t [36], u32);
# else
/*
 * NAME:	imdct36
...
void III_overlap(mad_fixed_t const output[36], mad_fixed_t overlap[18],
		 mad_fixed_t sample[18][32], u32 sb)
{
  u32 i;
 ...
 */
static
void III_freqinver(mad_fixed_t sample[18][32], u32 sb)
{
  u32 i;
...
s32 mad_layer_III(struct mad_stream *stream, struct mad_frame *frame)
{
  ...
  u32 si_len, data_bitlen, md_len;
  u32 frame_space, frame_used, frame_free;
