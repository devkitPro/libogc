...
$Id: irq.c,v 1.16 2005-12-09 09:35:45 shagkur Exp $
...
static void __irq_dump(u32 irqmask,u32 irq_idx)
{
	s32 len;
	u8 cpStr[256];
	static u32 irqidx = 0;
	...
	if((irqmask!=imask || irqidx!=irq_idx)) {
		strcpy(cpStr,"irqmask = ");

		len = strlen(cpStr);
		__irq_printHex(irqmask,&cpStr[len]);
		strcat(cpStr," irq_idx = ");
