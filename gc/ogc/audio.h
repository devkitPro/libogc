...
$Id: audio.h,v 1.4 2005-11-21 10:51:21 shagkur Exp $
...
\brief Register a callback function for the DVD audiostreaming interface
...
\param[in] rate streaming sample rate (AI_SAMPLERATE_32KHZ,AI_SAMPLERATE_48KHZ)
...
\return streaming sample rate (AI_SAMPLERATE_32KHZ,AI_SAMPLERATE_48KHZ)
*/
u32 AUDIO_GetStreamSampleRate();


/*! \fn AIDCallback AUDIO_RegisterDMACallback(AIDCallback callback)
\brief Register a callback function for the audio DMA interface.
...
\param[in] startaddr start address of the memory region to load into the audio DMA. NOTE: Has to be aligned on a 32byte boundery!
\param[in] len lenght of data to load into the audio DMA.
...
\brief Start the, previously with AUDIO_InitDMA() initialized, audio DMA operation.
